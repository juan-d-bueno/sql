...
SELECT *, cp.quantity*cp.cost_to_customer_per_qty AS price 
...
SELECT vendor_id, COUNT (vendor_id) assignments
...
CREATE TABLE temp.purchases_by_m_y AS
SELECT *,
strftime('%m', market_date) AS month,
strftime('%Y', market_date) AS year
FROM customer_purchases;
...
SELECT customer_id,
SUM(quantity*cost_to_customer_per_qty) AS Spending_Apr_2022,
month,
year
FROM purchases_by_m_y
